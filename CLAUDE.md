...
- **Меняешь `config.js`, `js/*` или `css/style.css` — подними `?v=ГГГГММДД`
  у ВСЕХ ссылок в `index.html` и `privacy.html`.** GitHub Pages отдаёт
  статику с `max-age=600`, поэтому без этого вернувшийся посетитель
  получает свежий HTML со старым JS. Так после удаления услуг из квиза
  они продолжали показываться в шаге 1 у тех, кто заходил раньше.
